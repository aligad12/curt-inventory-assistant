...
| **Google Gemini (`gemini-2.0-flash`) via `google-genai` SDK** | LLM with function calling | Supports native tool/function calling required by the task, and offers a free tier suitable for a student project with a tight deadline. The SDK auto-generates tool schemas directly from Python function signatures and docstrings, minimizing hand-written boilerplate. |
